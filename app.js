...
app.on('ready', function() {
    mainWindow = new BrowserWindow({
        ...
        webPreferences: {
            ...
            preload: path.join(original_dir, "preload.js") // use a preload script
        },
    });
...
function log(message){
    if(debugWindow){
        debugWindow.webContents.send("fromMain", message)
    } else {
        console.log(message)
    }
}
...
function replaceAssets(name){
    ...
    for (const key in assets) {
        ...
        if(key == "toPack"){
            ...
            fs.copyFileSync(toReplace, path.join(__dirname, "/tmp", toReplace_simplified))
            var tmpFilePath = toReplace_simplified

            let newBuffer = fs.readFileSync(toCopy);
            fs.writeFileSync(toReplace, newBuffer)
            replace.assets.push([tmpFilePath, paths[1]])
        }
    }
...
ipcMain.on("toMain", (event, args) => {
    ...
    if(args[0] == "exitClicked"){// Exit the window
        mainWindow.close();
